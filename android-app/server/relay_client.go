...
// RelayClient connects to the relay server with AES-GCM encryption
...
// RelayMessage represents a message exchanged with the relay server
...
// NewRelayClient creates a new relay client with AES-GCM encryption
func NewRelayClient(relayURL, key string, localPort int) *RelayClient {
	...
	// Compute and log webhook URL
	webhookHash := sha256.Sum256([]byte(key))
	webhookID := hex.EncodeToString(webhookHash[:])[:48]
	log.Printf("Relay webhook URL: https://minerva-relay.fly.dev/hook/%s", webhookID)
...
// Start begins the relay client connection loop in a goroutine
func (rc *RelayClient) Start() {
	go rc.connectLoop()
}

// Stop shuts down the relay client
...
func (rc *RelayClient) handleMessages() {
	// Keepalive ticker - send ping every 15 seconds
